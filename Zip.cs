...
internal static class Zip
{
    public static string ZipFiles(string filePath, string skinDirectory, string skinName)
    {
        ...
        var filesToZip = filesWork.Where(file => Regex.IsMatch(Path.GetFileName(file).ToLower(), filterPattern))
            .ToList();

        // Add MainBody.Mesh.gbx to list of files
        filesToZip.AddRange(filesSkin.Where(file => !string.Equals(Path.GetFileName(file), (skinName + ".Mesh.Gbx"),
            StringComparison.CurrentCultureIgnoreCase)).ToList());

        if (!filesToZip.Any()) return string.Empty;
